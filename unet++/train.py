...
class MYDATA(Dataset):
    ...
    def __getitem__(self, index):
        ...
        labels = np.array(labels).reshape(128, 128,1)
        labels = labels / 255.
...
# mynet = DenseUNet(1,downsample=True).to(device)
mynet = NestedUNet().to(device)
...
for epoch in range(num_epochs):
    ...
    if val_loss<min_loss:
        min_loss=val_loss
        torch.save(mynet.state_dict(),'NestedUnetmodel1.pkl')
