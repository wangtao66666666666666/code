...
class UNet_3Plus(nn.Module):
    ...
    def __init__(self, in_channels=3, n_classes=1, feature_scale=4, is_deconv=True,
                 is_batchnorm=True):  # in_channels=3表示输入的是彩色图
        ...
        self.h1_PT_hd4 = nn.MaxPool2d(8, 8,
                                      ceil_mode=True)  # MaxPool2d(kernel_size, stride, ceil_mode) kernel_size指最大池的窗口大小，stride是一次移动的步长，ceil_mode是向上取整。
        ...
        self.hd4_UT_hd3_conv = nn.Conv2d(self.UpChannels, self.CatChannels, 3,
                                         padding=1)  # 注意此处卷积的输入channels为UpChannels,即Decoder4聚合后的特征通道数量。
...
class UNet_3Plus_DeepSup(nn.Module):
    ...
    def forward(self, inputs):
        ...
        h1_PT_hd4 = self.h1_PT_hd4_relu(self.h1_PT_hd4_bn(
            self.h1_PT_hd4_conv(self.h1_PT_hd4(h1))))  # 与encoder1连接，relu( bn( conv( maxpool( h1 ) ) ) )
        ...
        h4_Cat_hd4 = self.h4_Cat_hd4_relu(
            self.h4_Cat_hd4_bn(self.h4_Cat_hd4_conv(h4)))  # 同层连接-与encoder4连接，relu( bn( conv( h4 ) ) )
        hd5_UT_hd4 = self.hd5_UT_hd4_relu(self.hd5_UT_hd4_bn(
            self.hd5_UT_hd4_conv(self.hd5_UT_hd4(hd5))))  # 与decoder5连接，relu( bn( conv( upsample ) ) )
        hd4 = self.relu4d_1(self.bn4d_1(self.conv4d_1(
            torch.cat((h1_PT_hd4, h2_PT_hd4, h3_PT_hd4, h4_Cat_hd4, hd5_UT_hd4),
                      1))))  # hd4->40*40*UpChannels   #特征聚合机制 relu( bn( conv( cat( 五个特征图 ) ) ) ) ，  1表示按维度1拼接，即按行拼接。
...
class UNet_3Plus_DeepSup_CGM(nn.Module):
    ...
    def forward(self, inputs):
        ...
        cls_branch = self.cls(hd5).squeeze(3).squeeze(
            2)  # (B,N,1,1)->(B,N)  #操作(dropout, conv1*1, adaptiveMaxPool, Sigmoid)后，产生一个二维张量  squeeze(x)只有当维度x的值为1时，才能去掉该维度。
        cls_branch_max = cls_branch.argmax(
            dim=1)  # dim=1将1维去掉，返回最大值对应的索引。  通过argmax，分类结果被转为一个单一数字输出。  #argmax(a, axis=None, out=Nont):a为输入的数组；axis=0按列寻找，axis=1按行寻找最大值对应的索引；out结果将被插入到a中。
        ...
        hd4 = self.relu4d_1(self.bn4d_1(self.conv4d_1(
            torch.cat((h1_PT_hd4, h2_PT_hd4, h3_PT_hd4, h4_Cat_hd4, hd5_UT_hd4),
                      1))))  # hd4->40*40*UpChannels   #1表示按维度1拼接，即按列拼接，即列变多。
